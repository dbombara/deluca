...
f_ig1, ax1 = plt.subplots(1, 3, figsize=(20, 4)) # Create a figure and axis for GPC
ax1[0].plot(tv,xv_gpc[0,:],label="$x_1$")
ax1[0].plot(tv,xv_gpc[1,:],label="$x_2$")
ax1[0].plot(tv, np.zeros_like(tv),label="$x_{1 \mid {ref}}$")
ax1[0].plot(tv, np.zeros_like(tv),label="$x_{2 \mid {ref}}$",linestyle="--")
...
ax1[1].plot(tv, np.zeros_like(tv),label="$x_{2 \mid {ref}}$",linestyle="--")
